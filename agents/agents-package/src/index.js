...
class MultiAgentSystem {
  ...
  registerAgents() {
    for (const [agentId, agent] of this.agents) {
      console.log(`Registering agent ${agentId}, agent.agentId: ${agent.agentId}, agent.name: ${agent.name}`);
      
      if (agent.agentId === undefined || agent.agentId === null) {
        console.error(`Agent ${agentId} missing agentId property!`, agent);
        throw new Error(`Agent ${agentId} is missing agentId property`);
      }
